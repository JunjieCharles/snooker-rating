...
根目录 `*.html`、`package.json`、`assets/*.js` 和 `assets/*.css` 中的前端代码按 [`LICENSE-CODE`](LICENSE-CODE) 所载 MIT License 授权。`assets/flags/` 中的旗帜来自 lipis/flag-icons v7.3.2，适用独立的 MIT [`LICENSE-FLAGS`](LICENSE-FLAGS)。目前不对 `data/**`、Snooker Rating Logo 或其他图片和矢量素材授予许可证；这些文件为网站运行而公开可读，但不属于 MIT License 的适用范围。

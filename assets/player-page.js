...
import { RatingChart } from "./chart.js?v=caa0dbaa7e6a";
import { formatNumber, t } from "./i18n.js?v=caa0dbaa7e6a";
import { filterMatches, formatRating, hasPublicMatchSource, pageRows, publicMatchSourceUrl } from "./state.js?v=caa0dbaa7e6a";
...
try {
  ...
  setPlayerName(document.querySelector("#breadcrumb-player"), profile);
  setPlayerName(document.querySelector("#player-name"), profile);
  const englishName = document.querySelector("#player-name-en");
  englishName.textContent = englishPlayerName(profile);
  englishName.hidden = !englishName.textContent;
  document.querySelector("#player-subtitle").replaceChildren(nationalityElement(profile), ` · ${profile.active_rank ? t("player.activeRank", { rank: profile.active_rank }) : t("player.inactive")}`);
  ...
  const renderChart = () => chart.render([{ player_key: key, name: profile.name, name_en: profile.name_en, points: curve.points }], { years, height: 510 }); renderChart();

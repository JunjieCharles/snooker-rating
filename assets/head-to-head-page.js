import { englishPlayerName, setPlayerName, fetchJson, hydrateShell, linkWithText, matchupScoreCell, nationalityElement, playerHref, setError } from "./common.js?v=caa0dbaa7e6a";
import { comparisonSeriesColors, RatingChart, seriesColor } from "./chart.js?v=caa0dbaa7e6a";
import { currentLocale, formatNumber, t } from "./i18n.js?v=caa0dbaa7e6a";
import {
  formatRating,
...
function renderUpcoming(snapshot) {
  ...
  const cards = matches.map((match) => {
    ...
    playerA.title = englishPlayerName(match, "player_a_name");
    ...
    playerB.title = englishPlayerName(match, "player_b_name");
...
class PlayerPicker {
  ...
  choose(key) {
    ...
    this.input.title = englishPlayerName(player);
    this.close();
  }
  ...
  render(query, activeIndex = -1) {
    ...
    if (!this.matches.length) {
      ...
      this.results.replaceChildren(...this.matches.map((player, index) => {
        ...
        const details = document.createElement("small");
        details.replaceChildren(nationalityElement(player), ` · ${playerActivityLabel(player)} · ${formatRating(player.rating)}`);
...
function playerCard(profile, color = seriesColor(profile.player_key)) {
  ...
  heading.append(linkWithText(playerHref(profile.player_key), profile.name, "", englishPlayerName(profile)));
  const rating = document.createElement("strong");
  rating.textContent = formatRating(profile.current_rating);
  const detail = document.createElement("small");
  detail.replaceChildren(nationalityElement(profile), ` · ${playerActivityLabel(profile)}`);
...
function legendItem(profile, wins, total, color, side) {
...
function ringCard(title, total, winsA, winsB, draws, profileA, profileB) {
  ...
  legend.append(legendItem(profileA, winsA, total, colorA, "left"), legendItem(profileB, winsB, total, colorB, "right"));
...
function scoreCard(bestOf, scores, profileA, profileB) {
  ...
  tbody.append(...scores.map((score, index) => {
    ...
    row.append(cell(formatNumber(index + 1)), matchupScoreCell(profileA.name, score.score_a, score.score_b, profileB.name, englishPlayerName(profileA), englishPlayerName(profileB)), cell(percent(score.probability), "numeric"));
    return row;
  }));
...
function renderMeetings(meetings, profileA, profileB) {
  ...
  body.replaceChildren(...meetings.map((match) => {
    ...
    row.append(cell(playedOn), tournament, setPlayerName(cell(winner, "match-result"), match.result === "win" ? profileA : match.result === "loss" ? profileB : { name: winner }), matchupScoreCell(profileA.name, match.score_for, match.score_against, profileB.name, englishPlayerName(profileA), englishPlayerName(profileB)), source);
    return row;
  }));
...
async function compare(playerAKey, playerBKey) {
  ...
  try {
    ...
    chartSeries = [
      { player_key: playerAKey, name: profileA.name, name_en: profileA.name_en, points: curveA.points, color: colorA },
      { player_key: playerBKey, name: profileB.name, name_en: profileB.name_en, points: curveB.points, color: colorB },
    ];

...
export function filterRanking(rows, query) {
  ...
  return rows.filter((row) => normalizeSearchText(`${row.name} ${row.name_en || ""} ${row.name_zh || ""} ${(row.name_aliases_zh || []).join(" ")} ${(row.name_aliases_en || []).join(" ")} ${row.nationality} ${row.nationality_en || ""} ${row.nationality_zh || ""} ${row.source_nationality || ""}`).includes(needle));
}
...
export function searchPlayers(players, query, limit = 12) {
  ...
  return findHeadToHeadPlayers(players, query, limit);
}
...
export function pointTime(point) {
  ...
  if (point.rating_time_policy === "calendar_day") return base;
...
export function filterMatches(matches, { query = "", year = "", result = "" } = {}) {
  ...
  return matches.filter((match) => {
    ...
    return normalizeSearchText(`${match.tournament_name} ${match.tournament_name_en || ""} ${match.tournament_name_zh || ""} ${match.stage} ${match.opponent_name} ${match.opponent_name_en || ""} ${match.opponent_name_zh || ""} ${(match.opponent_name_aliases_zh || []).join(" ")} ${(match.opponent_name_aliases_en || []).join(" ")}`).includes(needle);
  });
}
...
export function findHeadToHeadPlayers(players, query = "", limit = 20) {
  const needle = normalizeSearchText(query);
  return players
    .filter((player) => !needle || normalizeSearchText(`${player.name} ${player.name_en || ""} ${player.name_zh || ""} ${(player.name_aliases_zh || []).join(" ")} ${(player.name_aliases_en || []).join(" ")} ${player.nationality} ${player.nationality_en || ""} ${player.nationality_zh || ""} ${player.source_nationality || ""} ${player.player_key}`).includes(needle))
    .sort((left, right) => {
      const leftActive = Number.isInteger(left.active_rank);
      const rightActive = Number.isInteger(right.active_rank);
      if (leftActive !== rightActive) return leftActive ? -1 : 1;
      const leftRating = Number.isFinite(left.rating) ? left.rating : -Infinity;
      const rightRating = Number.isFinite(right.rating) ? right.rating : -Infinity;
      if (leftRating !== rightRating) return leftRating > rightRating ? -1 : 1;
      return left.player_key.localeCompare(right.player_key);
    })
    .slice(0, limit);
}

import { formatRating, searchPlayers } from "./state.js?v=074854fc008a";
import { applyLocale, currentLocale, formatNumber, setupLanguageSwitcher, t } from "./i18n.js?v=074854fc008a";
import { englishPlayerName, localizePlayerData, setPlayerName } from "./player-display.js?v=074854fc008a";
export { englishPlayerName, setPlayerName };
...
if (wechatFollow) {
  const summary = wechatFollow.querySelector("summary");
  const canHover = () => window.matchMedia("(hover: hover)").matches;
  wechatFollow.addEventListener("pointerenter", () => {
    if (canHover()) wechatFollow.open = true;
  });
  wechatFollow.addEventListener("pointerleave", () => {
    if (canHover()) wechatFollow.open = false;
  });
  summary.addEventListener("click", (event) => {
    if (canHover() && event.detail > 0) event.preventDefault();
  });
  wechatFollow.addEventListener("focusin", () => {
    if (canHover()) wechatFollow.open = true;
  });
  wechatFollow.addEventListener("focusout", (event) => {
    if (!wechatFollow.contains(event.relatedTarget)) wechatFollow.open = false;
  });
  document.addEventListener("click", (event) => {
    if (!wechatFollow.contains(event.target)) wechatFollow.open = false;
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && wechatFollow.open) {
      summary.focus();
      wechatFollow.open = false;
    }
  });
}
...
export async function fetchJson(path) {
  ...
  return localizePlayerData(await response.json(), currentLocale());
}
...
export function nationalityElement(player) {
  const span = document.createElement("span");
  span.className = "nationality-label";
  const code = player.flag_code;
  if (typeof code === "string" && /^(?:[a-z]{2}|gb-(?:eng|nir|sct|wls))$/.test(code) && code !== "tw") {
  ...
  span.append(document.createTextNode(player.nationality));
  return span;
}
...
function resultLink(player) {
  ...
  const name = setPlayerName(document.createElement("strong"), player);
  const detail = document.createElement("small");
  detail.append(nationalityElement(player), ` · ${formatRating(player.rating)}`);
...
export function linkWithText(href, text, className = "", title = "") {
  const link = document.createElement("a");
  link.href = href;
...
export function matchupScoreCell(playerA, scoreA, scoreB, playerB, englishA = "", englishB = "") {
  ...
  if (englishA) nameA.title = englishA;
  ...
  if (englishB) nameB.title = englishB;

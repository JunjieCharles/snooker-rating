import { englishPlayerName, setPlayerName, fetchJson, linkWithText, nationalityElement, playerHref } from "./common.js?v=074854fc008a";
import { RatingChart, widgetSeriesColors } from "./chart.js?v=074854fc008a";
import { t } from "./i18n.js?v=074854fc008a";
import { filterRanking, formatRating, pageRows, sortRows, toggleSelection } from "./state.js?v=074854fc008a";
...
export function setupRankingPage({ rows, cutoffDate, endSequence = null }) {
  ...
  function renderTable() {
    ...
    body.replaceChildren(...paged.rows.map((row) => {
      ...
      const name = document.createElement("td"); name.append(linkWithText(playerHref(row.player_key), row.name, "player-link", englishPlayerName(row)));
      const nationality = document.createElement("td"); nationality.append(nationalityElement(row));
      tr.append(compare, rank, name, nationality, tableCell(formatRating(row.rating), "numeric rating-cell"), tableCell(row.last_played), tableCell(row.matches, "numeric"));
      return tr;
    }));
    count.textContent = t("ranking.playerCount", { count: filtered.length });
    renderPagination(pagination, page, paged.pages, (next) => {
      const paginationTop = pagination.getBoundingClientRect().top;
      page = next;
      renderTable();
      // Keep the controls in place even when the last page has fewer rows.
  ...
  function renderSelection() {
    ...
    selectedRoot.replaceChildren(...keys.map((key) => {
      ...
      const label = document.createElement("span"); setPlayerName(label, row || { name });

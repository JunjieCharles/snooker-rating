...
The frontend code in the root `*.html` files, `package.json`, `assets/*.js`, and `assets/*.css` is licensed under the MIT License in [`LICENSE-CODE`](LICENSE-CODE). Flags in `assets/flags/` are from lipis/flag-icons v7.3.2 under the separate MIT [`LICENSE-FLAGS`](LICENSE-FLAGS). No license is currently granted for `data/**`, the Snooker Rating logos, or other image and vector assets; those files are publicly readable for operating the website but are not covered by the MIT License.
